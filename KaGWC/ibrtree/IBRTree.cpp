...
{
	map<int, vector<int>*> ::iterator it = indexNodes.begin();
	for (; it != indexNodes.end(); it--)		//for each non-leaf node. from bottom to up
	{
		map<int, vector<int> *> inverted;

		int indexID = it->first;
...
void IBRTree::BuildBTIndex()
{
	...
	map<int, vector<int> *>::iterator iterIndex = indexNodes.begin();
